...
class SharedPrefManager private constructor(private val mCtx: Context) {
    ...
    fun updateBalance(cashBal: String, chipsBal: String) {

        val sPref = mCtx.getSharedPreferences(SHARED_PREF_NAME, Context.MODE_PRIVATE)
        val editor = sPref.edit()
        if (cashBal.isNotEmpty()) {
            editor.putFloat("AcBal", cashBal.toFloat())
        }
        if (chipsBal.isNotEmpty()) {
            editor.putString("AcChipsBal", chipsBal)
        }
        editor.apply()
    }
