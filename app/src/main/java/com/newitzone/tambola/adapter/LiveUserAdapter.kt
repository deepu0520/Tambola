...
class LiveUserAdapter(val items : List<String>, val context: Context) : RecyclerView.Adapter<ViewHolderLiveUser>() {
    ...
    override fun onBindViewHolder(holder: ViewHolderLiveUser, position: Int) {
        holder?.tvUserName?.text = items.get(position)
    }
}

class ViewHolderLiveUser (view: View) : RecyclerView.ViewHolder(view) {
    // Holds the TextView that will add each animal to
    val tvUserName = view.text_user_name
}